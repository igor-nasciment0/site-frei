...
- **Curso** (`GET /courses`, e `GET /courses/count` → `{ total }` para a contagem pública): `id`, `code`, `name`, `type`, `workload`, `minAge`/`maxAge` (strings com formato "X anos ..."), `minSchoolLevel`, `contribution`, `imageId`, `description` (HTML), `jobMarket` (HTML opcional), `availablePeriods[]` (`name`, `entryTime`, `exitTime`, `isActive`).
...
- **Pagamento** (`POST /enrollments/my-enrollment/payment` e `GET …/payment/status`): `correlationId`, `status` (`1` pendente, `2` pago, `3` vencido, `4` recusado, `5` cancelado), `amount`, `copyPasteCode`, `qrCodeImageUrl` (URL da imagem, servida pelo provedor), `expiresAt`, `paidAt`. Emitido pelo backend e reaproveitado enquanto válido; o provedor não tem webhook, então a confirmação vem da consulta de status.
